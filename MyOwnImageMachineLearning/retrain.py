...
def create_image_lists(image_dir, testing_percentage, validation_percentage):
  ...
  """
  #Check if directory to images exists
  ...
  for sub_dir in sub_dirs:
    #First given is the main directory
    if is_root_dir:
      is_root_dir = False
      continue
    ...
    #Creation for each extension. WHY???
    ...
    for file_name in file_list:
        ...
        # Removal of _nohash_, $ signs etc from the file name
        hash_name = re.sub(r'_nohash_.*$', '', file_name)
        #decides if to put it in validation, testing or training images
        ...
        #returns an array with the label abd a dictionary of each label, contating the directory, an array of the training images, array for the testing images and another for validation images
    result[label_name] = {
        'dir': dir_name,
...
def get_image_path(image_lists, label_name, index, image_dir, category):
    ...
    """
    #make sure type (folder name) in list
    if label_name not in image_lists:
        tf.logging.fatal('Label does not exist %s.', label_name)
    label_lists = image_lists[label_name]

    #Double check if category is in list, i.e. testing, training, validation
    if category not in label_lists:
        tf.logging.fatal('Category does not exist %s.', category)
    category_list = label_lists[category]

    #Make sure more than 0 images in the chosen category
    if not category_list:
        tf.logging.fatal('Label %s has no images in the category %s.', label_name, category)

    #Find path to the given file
...
def create_model_graph(model_info):
    ...
    with tf.Graph().as_default() as graph:
        #find path to model graph and opem it 
...
def create_bottleneck_file(bottleneck_path, image_lists, label_name, index, image_dir, category, sess, jpeg_data_tensor, decoded_image_tensor, resized_input_tensor, bottleneck_tensor):
    ...
    #Find the path to the image and make sure is real
    image_path = get_image_path(image_lists, label_name, index, image_dir, category)
    if not gfile.Exists(image_path):
        tf.logging.fatal('File does not exist %s', image_path)
    #Get image data and create bottleneck
...
if __name__ == '__main__':
  ...
  #Might want to do for robot, not done in datagen
  parser.add_argument(
      '--random_brightness',
      type=int,
      default=0,
  ...
  #Read help, cant be bothered now, best to leave, but v. long
  parser.add_argument(
      '--architecture',
      type=str,
